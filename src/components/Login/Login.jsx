...
const Login = () => {
	...
	const loginUsingGitHub = async () => {
		try {
			...
			const response = await dispatch(loginWithGitHub(email))
			if (response.data) {
	...
	const loginUsingGoogle = async () => {
		try {
			...
			console.log("Data Google user", userData)
			// continua el flujo de authenticacion en la base de datos
		} catch (error) {
			...
			<div className="flex items-center justify-center h-screen Center">
				<div className="min-w-fit flex-col border bg-white px-6 py-14 shadow-md rounded-[4px] w-1/4 ">
					...
					<div className="mt-5 flex justify-between text-sm text-gray-600">
						<button onClick={()=> console.log("funcionalidad futura")}>Forgot password?</button>
