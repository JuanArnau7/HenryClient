...
export const getUsers = () => {
	return async (dispatch) => {
		try {
			...
			console.log("Error Redux actions on get active users", error.message);
			return error
		}
	}
}
export const getAllUsers = () => {
	return async (dispatch) => {
		try {
			const response = await axios.get(`${URL_SERVER}users/allUsers`);
			return dispatch({
				type: GET_TOTAL_USERS,
				payload: response.data
			})
		} catch (error) {
			console.log("Error Redux actions on get all users", error.message);
			return error
		}
	}
}
...
export const updateFood = (id, data) => {
	return async dispatch => {
		try {
			const res = await axios.put(`${URL_SERVER}foods/${id}`, data)
			return dispatch({
				type: UPDATE_FOOD,
				payload: res
			})
		} catch (error) {
			console.log('Error redux action on update food', error);
			return error.response
		}
	}
